...
function  addAirlineToFilter(row) {
	...
	if (exist === false) {
		...
		var _newNode = '<li><input type="checkbox" name="selectedAir" data-aircode="' + row.aircode + '" id="slAir' + _logo_img + '"><label for="slAir' + _logo_img + '">&nbsp;<img src="'+base_url+'airline_logo/sm' + _logo_img + '.gif" title="' + _airline + '">' + _airline + '</label></li>';
...
function add_to_sidebar(group, way, index){
	...
	html = '<h4 class="way">'+caption+': '+'<span class="fa fa-sort-desc pull-right" toggle="detail">'+accounting.formatMoney(_tic.subtotal)+'</span></h4>';
	html += '<div aria="detail" class="hidden">';
